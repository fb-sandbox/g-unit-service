...
public final class NHTSAMapper {
    ...
    /**
     * Convert NHTSA VIN decode response to Vehicle (vehicle data only, no unit association fields).
     *
     * @param response The NHTSA response
     * @param vin The VIN
     * @return The vehicle entity, or null if response is empty
     */
    public static Vehicle toVehicle(NHTSAVinDecodeResponse response, String vin) {
        if (response == null || response.getResults() == null || response.getResults().isEmpty()) {
            return null;
        }
